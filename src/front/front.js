    ...
    .then((d) => {
      ...
      d.forEach((inhab) => {
        ...
        card.addEventListener('click', () => {
          const hexa = hexagons.find(h => h.coord.x === inhab.Position.x
            && h.coord.y === inhab.Position.y);
          if (hexa) {
            drawMap(map);
...
inputSearch.addEventListener('change', (e) => {
  ...
  const search = () => cards.forEach((element) => {
    if (!element.cardObject.data.Name
      ...
      element.cardObject.hide(true);
    } else {
      element.cardObject.hide(false);
    }
  });
...
markmap.load('/map', MAP_WIDTH, MAP_HEIGHT, 0, 0).then(() => {
  drawMap(markmap);

  fetchMetadata().then((metadata) => {
    for (let row = 1; row <= 71; row += 1) {
      for (let col = 0; col < metadata['row-length'][`${row}`]; col += 1) {
    ...
    canvas.addEventListener('mousemove', (e) => {
      ...
      const hexa = hexagons.find(h => h.isIn(x, y));
      if (hexa) {
        drawMap(map);
        hexa.draw(drawer, zoomer, zoomOffset());
      }
    ...
    canvas.addEventListener('click', () => {
      const hexa = hexagons.find(h => h.active);
      if (hexa) {
        getSurvivors(hexa.coord.x, hexa.coord.y);
      }
    });
    ...
    rescount.addEventListener('click', () => {
      fetch(Requests.DetailsRequest(resource)).then(f => f.json()).then((details) => {
        ...
        details.forEach((detail) => {
          ...
          row.addEventListener('click', () => getSurvivors(detail[1], detail[2]));
